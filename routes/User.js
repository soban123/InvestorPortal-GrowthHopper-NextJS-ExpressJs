...
router.post('/', function(req, res, next) {
    ...
    const { name , email , password , role="investor" , amount , package } = req.body ; 
...
for( let t = 0 ; t < 31 ; t++ ){
  ...
  if( i == 0  ){
    if( t == 0 ){
      ...
      var returns = t % 2 == 0 ?   ( revenue / 30 ) + Math.floor(Math.random() * revenue/100 ) :    ( revenue/30  ) -  Math.floor(Math.random() * revenue/100 )   ;  
...
router.put('/block/:id', authenticateToken , function(req, res) {
    ...
    .then( result => res.json('User has been blocked') )
...
router.put('/active/:id', authenticateToken ,function(req, res) {
  ...
  User.findByIdAndUpdate(id , { status:1 })
  .then( result => res.json('User has been Active') )
...
if(email && password  ){
    ...
    if( user.length > 0 ){
      ...
      if( user[0].status == 1  ){
        ...
        var token = jwt.sign({ email: user[0].email , id: user[0]._id }, 'secretKey' ,  { expiresIn: '5h' });
...
let calPackageperc = package.split('')[0] ;
  ...
  router.get('/investorsdailyreturns/:email/:month',async function(req, res, next) {
  ...
  router.get('/investorsmonthlyreturns/:email/:month',async function(req, res, next) {
