...
const monthNames = ["January", "February", "March", "April", "May", "June",
  "July", "August", "September", "October", "November", "December"
    ...
    const Last7daysgraph = async() =>{
      ...
      let Currmonthdata = await Currentmonth.json();
       ...
       setstarttoendArr(arr)
        ...
        //Daily Data
            let dataarr = [];
            let response =   await  fetch('/user/investorsdailyreturns/'+email+"/"+(month));
            ...
            for ( let i = 0 ; i < 31 ; i++){  if( start + i > 30 ) {arr.push(  (start + i  ) - 30   ) } else{ arr.push( start + i ) }  } 
            ...
            if(NumberofDays > 6){
             ...
             dataarr =  dataarr.slice(NumberofDays-6 , NumberofDays+1)
              ...
              let resData =   await  fetch('/user/investorsdailyreturns/'+email+"/"+(month-1));
              let lastMonthDays = await resData.json();
              var newLastDays = [] ;
              if(lastMonthDays.length > 0 ){

                console.log( 'lastMonthDays[0]' , lastMonthDays[0]?.dailyprofit.slice(31-(7-NumberofDays) , lastMonthDays[0]?.dailyprofit.length ) ) 
                
                newLastDays = lastMonthDays[0]?.dailyprofit.slice(31-(7-NumberofDays) , lastMonthDays[0]?.dailyprofit.length ) 
              }
              else{
             ...
             console.log('data',dataarr)
              ...
              arr = [];
       for ( let i = 0 ; i < 7 ; i++){  arr.push(  (day - 6 + i  ))   } 


           setData({   labels:arr ,  datasets : [  { label:"Last 7 Days Returns" , backgroundColor: chartdata.datasets[0].backgroundColor , borderWidth: 1 , borderColor:chartdata.datasets[0].borderColor  , data:dataarr}]  })
    ...
    const handleLastMonthGraph = async () =>{
      ...
      console.log('TodayEarning'  ,Currmonthdata)
       ...
       setstarttoendArr(arr)
        ...
        //Daily Data
            ...
            console.log('sum',sum)
           setData({   labels:arr ,  datasets : [  { label:"Last Month Returns" , backgroundColor: chartdata.datasets[0].backgroundColor , borderWidth: 1 , borderColor:chartdata.datasets[0].borderColor  , data:dataarr}]  })
    ...
    const  thismonthgraph = async () => {
      ...
      setUserData(parsedUserData)
        //Last Month Data
      let responseLastmonth =   await  fetch('/user/investorsmonthlyreturns/'+parsedUserData.email+"/"+(month-1));
      let dataLastmonthly = await responseLastmonth.json();

      console.log('TodayEarning'  ,dataLastmonthly)
      setLastmonthdata(dataLastmonthly[0])


      let Currentmonth =   await  fetch('/user/investorsmonthlyreturns/'+parsedUserData.email+"/"+(month));
      let Currmonthdata = await Currentmonth.json();
     ...
     setstarttoendArr(arr)
      ...
      //Daily Data
          let dataarr = [];
          let response =   await  fetch('/user/investorsdailyreturns/'+parsedUserData.email+"/"+month);
          let data = await response.json();
          let sum = 0 ;
         ...
         if(todayprofit > 0) {setTodayEarning(todayprofit)} else { setTodayEarning(0) }

          
          const Yesterdayearn = dataarr.slice(startdate-1 , startdate) 
         if(Yesterdayearn > 0) {setYesterdayEarning(Yesterdayearn)} else { setYesterdayEarning(0) }
    ...
    console.log('starttoendArr',starttoendArr)
   ...
   const  handleSelectForDate = (ranges) => {
    ...
    const RangeGraph = async (strday , strtmnt , endday , endmnt  ) =>{
          ...
          if ( diffinmnth == 0 ){
            ...
            let responseRange =   await  fetch('/user/investorsdailyreturns/'+email+"/"+(strtmnt));
              ...
              let resendmnth = await  fetch('/user/investorsdailyreturns/'+email+"/"+(endmnt));
              ...
              for ( let k = 1 ; k <= endday ; k++){   arr2.push(  k )  } 


              let arr3 = arr.concat(arr2)

              console.log('TodayEarning'  ,resenddata)
              setData({   labels:arr3 ,  datasets : [  { label:"Daily Retunrs" , backgroundColor: chartdata.datasets[0].backgroundColor , borderWidth: 1 , borderColor:chartdata.datasets[0].borderColor  , data:dataarr}]  })
            ...
            for( let i = 0 ; i <= diffinmnth ; i++ ){
              ...
              if( i ==  0  ){
                let responseRange =   await  fetch('/user/investorsdailyreturns/'+email+"/"+(strtmnt+i));
                let resstrtdata = await responseRange.json();
                console.log('TodayEarning'  ,resstrtdata)
                ...
                for ( let i = 1 ; i <= endday ; i++){  if( i  > 30 ) {arr.push(  ( i  ) - 30   ) } else{ arr.push(  i ) }  } 
              }
              ...
              for ( let i = strday ; i < 31 ; i++){   arr.push(  i )  } 
            }
        ...
        <div className="right_col" role="main">
          ...
          <div className="tile_count">
            <div className="col-md-3 col-sm-4  tile_stats_count">
              <span className="count_top"><i className="fa fa-dollar"></i> Total Amount Invest</span>
                <div className="count green">{userData.amount}</div>
            </div>
            <div className="col-md-3 col-sm-4  tile_stats_count">
              <span className="count_top"><i className="fa fa-clock-o"></i> Today Earning </span>
            ...
            <div className="col-md-3 col-sm-4  tile_stats_count">
              <span className="count_top"><i className="fa fa-clock"></i> Yesterday  Earning</span>
    <div className="count purple">{YesterdayEarning}</div>
            ...
            <div className="col-md-3 col-sm-4  tile_stats_count">
              <span className="count_top"><i className="fa fa-dollar"></i> Last Month Earning</span>
                <div className="count orange">{lastmonthdata?.revenue ? lastmonthdata.revenue : 0}</div>
          ...
          <div className="row">
            <div className="col-md-12 col-sm-12 ">
              <div className="dashboard_graph">
                ...
                <DateRangePicker handleSelectForDate={handleSelectForDate} modalshow={modalshow} handleClose={handleClose} /> 
