...
//   let [password , setPassword] = useState('');
   let [userData , setUserData] =  useState({ id: '' , name: '' , email: '' , password : '' , amount : '' , package: '6%'  })
...
fetch('/user/block/'+id , {
  ...
  const handleView = (id)=>{
      fetch( '/user/'+id  )
      .then( res => res.json() )
      .then( data => {setUserData({ name:data.name ,
                                                        email:data.email ,
                                                        id: data._id ,
                              ...
                              users.map( (user , index )=>{
                                ...
                                <small>Created { user.date ?  user.date.split('T')[0] : 'not Specified'  }</small>
                                    ...
                                    <td>
                                      <button
                                        type="button"
                                        className= {  user.status == 1 ? "btn btn-success btn-xs" : "btn btn-danger btn-xs"  } 
                                      >
                                    ...
                                    <td>
                                      <button
                                        ...
                                        onClick={()=>{ setModalView("view") ; handleView(user._id) }}
                                      >
                                      ...
                                      <button
                                        ...
                                        onClick={()=>{ setModalView("update") ; handleView(user._id) }}

                                      >
                                        <i className="fa fa-pencil"></i> Edit
                                      </button>
                                     { user.status == 1 ? <button  onClick={ () => handleBlock(user._id)} className="btn btn-danger btn-xs"
                                        ><i className="fa fa-trash-o"></i> Block
                                      </button> 
                                    :   
                                    <button  onClick={ () => handleActive(user._id)} className="btn btn-success btn-xs"
          ...
          <div className="modal-dialog modal-lg">
            <div className="modal-content">
              <div className="modal-header">
                <h4 className="modal-title" id="myModalLabel">User View / Edit</h4>
                    ...
                    <div className="col-md-6 col-sm-6">
                      <select 
                           ...
                           class="custom-select">
                            ...
                            <option value={userData.package} >{userData.package}</option>
                          </select>
                          <div className={CRIShowHide == "show" ? "show" : "hide"}>
                              <p> Write your custom percentage :  </p>
                                ...
                                type="text"
                                disabled = {   (ModalView == "update") ?  false : true }
                                onChange={(e)=>setUserData({...userData , package: e.target.value })}
                                name="middle-name"
                                required
                                />
                          </div>
              ...
              <div className="modal-footer">
                ...
                { ModalView == "update" ?  <button type="button" onClick={ handleSubmit } className="btn btn-primary">
                  Save changes
                </button>
                 : ""   }
              </div>
